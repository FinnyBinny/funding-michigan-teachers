...
const EMBED_BASE = 'https://www.zeffy.com/en-US/embed/donation-form/supporting-the-teachers-who-support-us';
...
export default function DonationModal({ isOpen, onClose, amount, frequency = 'monthly' }: DonationModalProps) {
  ...
  // Zeffy supports ?amount=X&frequency=monthly|once
  const params = amount
    ? `?amount=${amount}&frequency=${frequency}`
    : '';
  const embedUrl = `${EMBED_BASE}${params}`;

  // Fallback link (opens new tab if iframe is blocked by the payment provider)
  const fallbackBase = 'https://www.zeffy.com/en-US/donation-form/supporting-the-teachers-who-support-us';
  const fallbackUrl = amount
    ? `${fallbackBase}?amount=${amount}&frequency=${frequency}`
    : fallbackBase;
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-2xl bg-white rounded-[1.5rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col"
            style={{ maxHeight: '92dvh' }}
            onClick={e => e.stopPropagation()}
          >
            {/* Header */}
            <div className="bg-chalkboard px-5 py-4 sm:px-8 sm:py-5 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-1">
                ...
                <button onClick={onClose} aria-label="Close donation modal" className="p-2 hover:bg-white/10 rounded-xl transition-colors text-white/60 hover:text-white">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            {/* Iframe area */}
            <div className="relative flex-1 overflow-auto bg-paper" style={{ minHeight: 'min(540px, 55dvh)' }}>
              {/* Loading spinner */}
              <AnimatePresence>
                {loading && (
                  <motion.div
              ...
              <iframe
                ...
                style={{ minHeight: 'min(540px, 55dvh)', height: '100%', display: 'block' }}
            ...
            <div className="px-6 py-3 bg-paper/80 border-t border-chalkboard/5 flex items-center justify-between shrink-0">
              <p className="text-[10px] text-chalkboard/30 font-medium">
                🔒 Payments processed securely by Zeffy · 100% tax-deductible · EIN 93-4485967
              </p>
